...
void calibrate() {
	...
	if(SD.exists("CALIBRAT.TXT")) SD.remove("CALIBRAT.TXT");
	...
	File cal = SD.open("CALIBRAT.TXT", FILE_WRITE);
	if(cal){
		...
		//SerialUSB.print("Error opening CALIBRAT.TXT");
	}
...
bool readCalibration() {
	...
	if(!SD.exists("CALIBRAT.TXT")) return false;
	
	File cal = SD.open("CALIBRAT.TXT");
	if(cal){
		// acceleration - x
		while(cal.available()) {
			rc = cal.read();
			...
			// list files on SD
				...
				while(true) {
					...
					if(!entry.isDirectory()) {
						...
						if(ename.equals("CALIBRAT.TXT")) continue;
...
void sensorRead(byte buff[22]){
	...
	// accelerometer x
	int ax = (int) ((a_x*6 + ca_x) * 2048);
	for(int i = 0; i < 2; i ++) {
		buff[5-i] = (byte) ((ax>>(i*8))&0xFF);
	}
	// accelerometer y
	int ay = (int) ((a_y*6 + ca_y) * 2048);
	for(int i = 0; i < 2; i ++) {
		buff[7-i] = (byte) ((ay>>(i*8))&0xFF);
	}
	// accelerometer z
	int az = (int) ((a_z*6 + ca_z) * 2048);
	for(int i = 0; i < 2; i ++) {
